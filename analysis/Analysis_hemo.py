...
from WF_functions import load_nidaq, illum_seq, get_tif_fr_ilu, load_and_filter, smooth_pool, animate_frameseq, extract_opto_frames, get_target_frames, plot_result, hemo_corr
#%%
...
#%%Hemo-correct whole recording
hs_frames = hemo_corr(ds_blue_frames, ds_uv_frames, color_seq, 1)
...
for i in range(len(bl_frames)):
    ...
    first = last - 12
    bl_frames[i,:,:] = np.mean(hs_frames[first:last,:,:], axis = 0)
...
result_frame1 = plot_result(animal+session+'_Frame_1',frame_1,hs_frames,bl_frames,save=saving,x=100,y=160,z=zi)
result_frame2 = plot_result(animal+session+'_Frame_2',frame_2,hs_frames,bl_frames,save=saving,x=100,y=160,z=zi)
result_frame3 = plot_result(animal+session+'_Frame_3',frame_3,hs_frames,bl_frames,save=saving,x=100,y=160,z=zi)

result_frame3last = plot_result(animal+session+'_Frame_-3',frame_3last,hs_frames,bl_frames,save=saving,x=100,y=160,z=zi)
result_frame2last = plot_result(animal+session+'_Frame_-2',frame_2last,hs_frames,bl_frames,save=saving,x=100,y=160,z=zi)
result_frame1last = plot_result(animal+session+'_Frame_-1',frame_1last,hs_frames,bl_frames,save=saving,x=100,y=160,z=zi)
#%
plt.figure()
plt.imshow(np.mean(hs_frames, axis=0), cmap='Greys')
if saving =='on':
    plt.savefig(animal+session+'_avg_frame')
#%%
np.save(animal+session, hs_frames)